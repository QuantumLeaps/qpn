/*****************************************************************************
* Product: QK-nano, port to ARM with GNU
...
/*****************************************************************************
* unsigned int QF_isr_disable_SYS(void);
*/
    .global QF_isr_disable_SYS
    .func   QF_isr_disable_SYS
QF_isr_disable_SYS:
    MRS     r0,cpsr             /* get the original CPSR in r0 to return */
    MSR     cpsr_c,#(SYS_MODE | NO_INT) /* disable both IRQ/FIQ */
    BX      lr                  /* return the original CPSR in r0 */

    .size   QF_isr_disable_SYS, . - QF_isr_disable_SYS
    .endfunc


/*****************************************************************************
* void QF_isr_restore_SYS(unsigned int key);
*/
    .global QF_isr_restore_SYS
    .func   QF_isr_restore_SYS
QF_isr_restore_SYS:
    MSR     cpsr_c,r0           /* restore the original CPSR from r0 */
    BX      lr                  /* return to ARM or THUMB */

    .size   QF_isr_restore_SYS, . - QF_isr_restore_SYS
    .endfunc
