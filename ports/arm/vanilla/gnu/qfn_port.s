...
/*****************************************************************************
* void QF_int_unlock_SYS(void);
*/
    .global QF_int_unlock_SYS
    .func   QF_int_unlock_SYS
QF_int_unlock_SYS:
    MSR     cpsr_c,#(SYS_MODE)  /* enable both IRQ/FIQ */
    BX      lr
...
/*****************************************************************************
* QF_ISR_KEY_TYPE QF_isr_lock_SYS(void);
*/
    .global QF_isr_lock_SYS
    .func   QF_isr_lock_SYS
QF_isr_lock_SYS:
    MRS     r0,cpsr             /* get the original CPSR in r0 to return */
    MSR     cpsr_c,#(SYS_MODE | NO_INT) /* disable both IRQ/FIQ */
    BX      lr                  /* return the original CPSR in r0 */

    .size   QF_isr_lock_SYS, . - QF_isr_lock_SYS
    .endfunc


/*****************************************************************************
* void QF_isr_unlock_SYS(QF_ISR_KEY_TYPE key);
*/
    .global QF_isr_unlock_SYS
    .func   QF_isr_unlock_SYS
QF_isr_unlock_SYS:
    MSR     cpsr_c,r0           /* restore the original CPSR from r0 */
    BX      lr                  /* return to ARM or THUMB */

    .size   QF_isr_unlock_SYS, . - QF_isr_unlock_SYS
    .endfunc
