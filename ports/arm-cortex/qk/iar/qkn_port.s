;*****************************************************************************
; Product: QK-nano for ARM Cortex-M0/M3, IAR ARM assembler, CMSIS-compliant
...
; preemptions in QK-nano. The use of the PendSV exception is the recommended
...
;
; Also, *all* ISRs in the QK-nano application must trigger the PendSV exception
; by calling the QK_ISR_EXIT() macro.
;
; Due to tail-chaining and its lowest priority, the PendSV exception will be
; entered immediately after the exit from the *last* nested interrupt (or
; exception). In QK-nano, this is exactly the time when the QK scheduler needs
; to check for the asynchronous preemptions.
;
;*****************************************************************************
PendSV_Handler:
...
SVC_Handler:
    ...
    BX      lr                ; exception-return to the preempted task
