/*****************************************************************************
* Product: PELICAN crossing example, MPLAB-C18 compiler
...
QState Ped_wait(Ped *me) {
    switch (Q_SIG(me)) {
        ...
        case Q_TIMEOUT_SIG: {
            if ((--me->retryCtr) != 0U) {
                QActive_arm((QActive *)me, WAIT_TOUT);
                QActive_post(&AO_Pelican, PEDS_WAITING_SIG, 0);
            }
            else {
...
QState Ped_off(Ped *me) {
    switch (Q_SIG(me)) {
        case Q_ENTRY_SIG: {
            ...
            QActive_post(&AO_Pelican, OFF_SIG, 0);
            return Q_HANDLED();
        }
        case Q_TIMEOUT_SIG: {
            QActive_post(&AO_Pelican, ON_SIG, 0);
            return Q_TRAN(&Ped_wait);
        }
    }
    return Q_SUPER(&QHsm_top);
