...
static QState Pelican_pedsFlash(Pelican * const me) {
    ...
    switch (Q_SIG(me)) {
        ...
        case Q_TIMEOUT_SIG: {
            /* ${AOs::Pelican::SM::operational::pedsEnabled::pedsFlash::Q_TIMEOUT::[me->flashCtr!~]} */
            if (me->flashCtr != 0U) {
                --me->flashCtr;
                QActive_armX(&me->super, 0U, PEDS_FLASH_TOUT);
                /* ${AOs::Pelican::SM::operational::pedsEnabled::pedsFlash::Q_TIMEOUT::[me->flashCtr!~]::[(me->flashCtr~]} */
                if ((me->flashCtr & 1U) == 0U) {
                    BSP_signalPeds(PEDS_DONT_WALK);
                    status_ = QM_HANDLED();
                }
                /* ${AOs::Pelican::SM::operational::pedsEnabled::pedsFlash::Q_TIMEOUT::[me->flashCtr!~]::[else]} */
                else {
...
static QState Pelican_offline(Pelican * const me) {
    ...
    switch (Q_SIG(me)) {
        ...
        case Q_TIMEOUT_SIG: {
            QActive_armX(&me->super, 0U, OFF_FLASH_TOUT);
            me->flashCtr ^= 1U;
            /* ${AOs::Pelican::SM::offline::Q_TIMEOUT::[(me->flashCtr~]} */
            if ((me->flashCtr & 1U) == 0U) {
                BSP_signalCars(CARS_RED);
                BSP_signalPeds(PEDS_DONT_WALK);
                status_ = QM_HANDLED();
